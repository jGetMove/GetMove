...
int main(int argc, char **argv) {
    ...
    if (cfg.DEBUG) {    // affiche des trucs dans ErrorFile.log si débug plus d'autres infos
    ...
    OccurenceDeriver fullOcc(
            fullDB); // trie dans OccurenceDeriver.table les transaction en fonction des clusters (l'inverse de Database.transactions)
    ...
    for (int i = 1; i <= num_block; i++) {
        ...
        lcm.RunLcmNew(fullDB, transactionsets, numItemsLevel2, lcmTimeIndex.item_temp, lcmTimeIndex.timeindex,
                      numIDlevel2, timeIDlevel2);
...
void Usage() {
    cout << endl
         ...
         << "\t min_sup stands for the minimal support [1..100], i.e. the number of objects embedded in the clusters"
         << endl
...
void RandomSampling(int numObject) {
    ...
    for (int i = 10; i <= 100; i = i + 10) {
        ...
        fstream file;
        string filename =
                "/home/phannhathai/Programming/IncrementalVersionLCM/OriginalFullSwain/randomSample/random" + s1 +
                ".dat";
