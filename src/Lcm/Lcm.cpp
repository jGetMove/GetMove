 ...
 *************************************************************************/
void Lcm::RunLcmNew(Database &database, vector<Transaction> &transactionsets, int &numItems, vector<int> &itemID,
                    vector<int> &timeID, vector<vector<int>> &level2ItemID, vector<vector<int>> &level2TimeID) {
    ...
    LcmIterNew(database, itemsets, transactionList, occ, freqList, transactionsets, numItems, itemID, timeID,
 ...
 *************************************************************************/
void Lcm::RunLcm(OccurenceDeriver &fullOcc, Database &database, vector<vector<int>> &level2ItemID,
                 vector<vector<int>> &level2TimeID) {
...
//
//}
void Lcm::GenerateItemsets(Database &database, vector<int> &itemsets, vector<int> &itemID, vector<int> &timeID,
                           vector<vector<int>> &generatedItemsets, vector<vector<int>> &generatedtimeID,
                           vector<vector<int>> &generateditemID, int &sizeGenerated) {
    ...
    for (unsigned int itemsetIndex = 0; itemsetIndex < PosDates.size(); ++itemsetIndex) {
        ...
        if (itemsetIndex == 0) {
            ...
            for (vector<vector<int> >::iterator it = generatedItemsets.begin(); it != generatedItemsets.end();
                 ++it) {
 ...
 *************************************************************************/
void Lcm::LcmIterNew(Database &database, vector<int> &itemsets, vector<int> &transactionList, OccurenceDeriver &occ,
                     vector<int> &freqList, vector<Transaction> &transactionsets, int &numItems, vector<int> &itemID,
                     vector<int> &timeID, vector<vector<int>> &level2ItemID, vector<vector<int>> &level2TimeID) {
    ...
    int sizeGenerated = 1;
    GenerateItemsets(database, itemsets, itemID, timeID, generatedItemsets, generatedtimeID, generateditemID,
                     sizeGenerated); // met itemsets (tableau vide) dans generatedItemsets
    ...
    for (unsigned int nbitemset = 0; nbitemset < generatedItemsets.size(); nbitemset++) {
        ...
        PrintItemsetsNew(generatedItemsets[nbitemset], occ, transactionsets, numItems, timeID, level2ItemID,
                         level2TimeID); // print $ itemset(qui est toujours vide)

        if (cfg.DEBUG) {
            cerr << "content of generatedItemset"
                 << endl; // generatedItemsets[nbitemset].size() = 0 car itemsets est vide
            for (unsigned int i = 0; i < generatedItemsets[nbitemset].size(); i++) {
                cerr << generatedItemsets[nbitemset][i] << " " << occ.GetNumOcc(generatedItemsets[nbitemset][i])
                     << endl;
            }
        }
        ...
        for (int i = *iter; iter != totalItem.end(); iter++, i = *iter) { // i = clusterId
            if ((int) occ.table[i].size() >= min_sup &&
                !binary_search(generatedItemsets[nbitemset].begin(), generatedItemsets[nbitemset].end(), i))
                freq_i.push_back(i);
        }
        ...
        for (vector<int>::iterator freq = freq_i.begin(); freq != freq_i.end(); freq++) { // pour chauqe ClusterID
            ...
            if (PpcTest(database, generatedItemsets[nbitemset], transactionList, *freq, newTransactionList)) {
                ...
                if (max_pat == 0 || (int) q_sets.size() <= max_pat) {
                    ...
                    LcmIterNew(database, q_sets, newTransactionList, occ, newFreqList, transactionsets, numItems,
                               itemID, timeID, level2ItemID, level2TimeID);
                }
            }
        }
...
void Lcm::ShowItemsetsInfos(vector<int> &itemsets, OccurenceDeriver &occ) {
    ...
    for (unsigned int i = 0; i < itemsets.size(); i++) {
        ...
        for (unsigned int j = 0; j < itemsets.size(); j++) {
            vector<int> table = occ.GetTable(itemsets[j]);
            cerr << " itemset : " << itemsets[j] << " objects : ";
 ...
 * main function
 *************************************************************************/
void Lcm::LcmIter(OccurenceDeriver &fullOcc, Database &database, vector<int> &itemsets, vector<int> &transactionList,
                  OccurenceDeriver &occ, vector<int> &freqList, vector<vector<int>> &level2ItemID,
                  vector<vector<int>> &level2TimeID) {
    ...
    */
    ...
    for (int i = *iter; iter != totalItem.end(); iter++, i = *iter) {
        if ((int) occ.table[i].size() >= min_sup && !binary_search(itemsets.begin(), itemsets.end(), i))
            freq_i.push_back(i);
    ...
    for (vector<int>::iterator freq = freq_i.begin(); freq != freq_i.end(); freq++) {
        ...
        if (PpcTest(database, itemsets, transactionList, *freq, newTransactionList) &&
            !binary_search(blocks.begin(), blocks.end(), blockOfItem) /*&& lengthItemsets > min_t*/) {
            q_sets.clear();
            if (cfg.DEBUG) {
                ...
                //also detect min_t
            {
                ...
                if (cfg.DEBUG) {
                    occ.Print(cerr);
                    cerr << "****************Normalement ici il a mis a jour newTransactionList et appel update"
                         << endl;
 ...
 * Update Freq List
 *************************************************************************/
inline void Lcm::UpdateFreqList(const Database &database, const vector<int> &transactionList, const vector<int> &gsub,
                                vector<int> &freqList, int freq, vector<int> &newFreq) {
    int iter = 0;
    if (freqList.size() > 0) {
    ...
    for (int i = iter; i < (int) gsub.size(); i++) {
        ...
        for (unsigned int list = 0; list < newList.size(); list++) {
            ...
            if (binary_search(transaction.itemsets.begin(), transaction.itemsets.end(), item)) {
                freqCount += 1;
 ...
 *************************************************************************/
inline void
Lcm::UpdateTransactionList(const Database &database, const vector<int> &transactionList, const vector<int> &q_sets,
                           int item, vector<int> &newTransactionList) {
    ...
    for (int i = 0; i < (int) transactionList.size(); i++) {
        ...
        for (iter = 0; iter < (int) q_sets.size(); iter++) {
            int q = q_sets[iter];
            if (q >= item) {
                if (!binary_search(transaction.itemsets.begin(), transaction.itemsets.end(), q))
                    break;
            }
        }
 ...
 ***************************************************************************/
inline void
Lcm::PrintItemsetsNew(const vector<int> &itemsets, const OccurenceDeriver &occ, vector<Transaction> &transactionsets,
                      int &numItems, vector<int> &timeID, vector<vector<int>> &level2ItemID,
                      vector<vector<int>> &level2TimeID) {
    ...
    if ((int) itemsets.size() > min_t)   // only retrieve itemsets which have length is larger than min_t
    {
        ...
        if (itemsets[0] == 7777777) {
            ...
            //Compress item and time into level 2
            for (unsigned int i = 0; i < itemsets.size(); i++) {
                itemsetsTemp.push_back(
                        itemsets[i]);//for Compo algorithm (Mining Compression Movement Patterns for Moving Objects)
            ...
            for (unsigned int i = 0; i < table.size(); i++) {
                ...
                transactionsets.at((unsigned int) table[i]).itemsets.push_back(
                        numItems); //put new index of new item (second level) (not all the first level information)
                //////////////////////////
            }
            if (cfg.DEBUG) {
                cerr << endl;
 ...
 ***************************************************************************/
inline void Lcm::PrintItemsets(OccurenceDeriver &fullOcc, const vector<int> &itemsets, const OccurenceDeriver &occ,
                               vector<vector<int>> &level2ItemID, vector<vector<int>> &level2TimeID) {
    ...
    if ((int) itemsets.size() > 0) {
        ...
        if (itemsets[0] == 7777777) {
            ...
            if (cfg.DEBUG) {
                cerr << "on met dans table le conenu de occ, la taille de table est table.size()" << table.size()
                     << endl;
 ...
 **************************************************************************/
inline bool Lcm::PpcTest(const Database &database, vector<int> &itemsets, vector<int> &transactionList, int item,
                         vector<int> &newTransactionList) {
    ...
    CalcTransactionList(database, transactionList, item,
                        newTransactionList); // mets dans newTransactionList l'ensemble des transactions contenant item (le custer id)

    // check j s.t j < i, j \notin P(i-1) is included in every transaction of T(P \cup {i})
    for (vector<int>::iterator j = totalItem.begin(); *j < item; j++) {
        if (!binary_search(itemsets.begin(), itemsets.end(), *j) &&
            CheckItemInclusion(database, newTransactionList, *j)) {
 ...
 *****************************************************************************/
inline void
Lcm::MakeClosure(const Database &database, vector<int> &transactionList, vector<int> &q_sets, vector<int> &itemsets,
                 int item) {
    ...
    for (int iter = *i; i != totalItem.end(); i++, iter = *i) {
        if (CheckItemInclusion(database, transactionList, iter)) {
...
inline bool Lcm::CheckItemInclusion(const Database &database, vector<int> &transactionList, int item) {
    for (vector<int>::iterator iter = transactionList.begin(); iter != transactionList.end(); iter++) {
        ...
        if (!binary_search(transaction.itemsets.begin(), transaction.itemsets.end(), item)) return false;
    }
    return true;
}
 ...
 *********************************************************************************/
inline void Lcm::CalcTransactionList(const Database &database, const vector<int> &transactionList, int item,
                                     vector<int> &newTransactionList) {

    for (int list = 0; list < (int) transactionList.size(); list++) { // pour chaque transaction
        const Transaction &transaction = database.database[transactionList[list]]; // on récupère la transaction
        if (binary_search(transaction.itemsets.begin(), transaction.itemsets.end(),
                          item)) // on verifie si l'id est dans l'itemset de la transaction
...
*****************************************************************/
void
Lcm::ConvoyDetect(OccurenceDeriver &fullOcc, vector<int> &timesets, vector<int> &ItemSets, vector<int> &objectsets) {
    ...
    for (unsigned int i = 0; i < timesets.size(); i++) {
        ...
        if (currentTime == (lastTime + 1)) {
            ...
            for (unsigned int j = 0; j < correctObjectsets.size(); j++) {
                if (binary_search(currentObjectsets.begin(), currentObjectsets.end(), correctObjectsets[j])) {
...
*****************************************************************/
void Lcm::GroupPatternDetect(OccurenceDeriver &fullOcc, vector<int> &timesets, vector<int> &ItemSets,
                             vector<int> &objectsets) {
    ...
    for (unsigned int i = 0; i < timesets.size(); i++) {
        ...
        for (unsigned int k = 0; k < currentObjectsets.size(); k++) {
            cerr << currentObjectsets[k] << " ";
        }
        cerr << endl;

        cerr << "correctObjectsets : ";
        for (unsigned int l = 0; l < correctObjectsets.size(); l++) {
            cerr << correctObjectsets[l] << " ";
        }
        cerr << endl;
    ...
    for (unsigned int i = 1; i < alltables.size(); i++) {
        ...
        if (isIncluded(alltables[i - 1], alltables[i]) == 1) {
            ...
            if (dis) {
                ...
                way.push_back(false);
                if ((unsigned int) lv2firsttime < i - 1 && lv2firsttime != -1) {
...
void Lcm::CloseSwarmDetect(OccurenceDeriver &fullOcc, vector<int> &timesets, vector<int> &ItemSets,
                           vector<int> &objectsets) {
    cout << "-----------------------------------------------" << endl;
...
*****************************************************************/
void Lcm::fCloseSwarmDetect(OccurenceDeriver &fullOcc, vector<int> &timesets, vector<int> &ItemSets,
                            vector<int> &objectsets) {
    ...
    for (unsigned int i = 0; i < timesets.size() - 1; i++) {
        ...
        if (gap <= 42 && gap >= 2) {
            ...
            if (strong / ((end - start + 1) * 1.0) >= min_w || medium / ((end - start + 1) * 1.0) >= min_w ||
                weak / ((end - start + 1) * 1.0) >= min_w) {
                end++;
                //cerr << (end - start) << endl;
                vector<int> currentObjectsets = fullOcc.GetTable(ItemSets[i]);
                vector<int> objectTemp;
