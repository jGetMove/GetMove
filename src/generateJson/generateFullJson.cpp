...
void parse(int argc, char **argv) {
    if (argc != 3) {
        ...
        cout
                << "initialdata.dat is the file in output of the lcm algorithm. Be careful the initialdatatimeindex.dat must be in the same directory"
                << endl;
        exit(0);
    }
    filename = argv[1];
...
std::vector<std::string> split(const std::string &str, const std::string &delim) {
    ...
    do {
        ...
        string token = std::string(begin, str_it); // grab the token
...
void ProcessPatterns(ifstream &filePathes, vector<int> &objects, vector<int> &times, map<int, vector<int> > &TheObjects,
                     map<int, vector<int> > &TheTimes, int &nbpattern) {
 ...
 */
    ...
    while (getline(filePathes, ligne)) {
        ...
        if (found != std::string::npos) {
            ProcessPatterns(filePathes, objectsConverge, timeConverge, MymapConvergeObject, MymapConvergeTimes,
                            nbConverge);
        }
